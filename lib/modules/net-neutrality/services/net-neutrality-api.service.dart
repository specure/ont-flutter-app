...
class NetNeutralityApiService extends DioService {
  ...
  Future<NetNeutralitySettingsResponse?> getSettings(
    ...
    try {
      final response = await dio.get('/netNeutralityTestRequest');
  ...
  Future<void> postResults(
    ...
    try {
      await dio.post('/netNeutralityResult', data: results);
    } on DioError catch (e) {
      print(e);
      errorHandler?.process(e);
      return null;
  ...
  Future<List<NetNeutralityHistoryItem>?> getHistory(String openTestUuid,
    ...
    try {
      ...
      final Response<Map<String, dynamic>> response = await dio.get(
          '/reports/netNeutralityResult/history?sort=measurementDate,desc&uuid=$uuid&openTestUuid=$openTestUuid');
  ...
  Future<NetNeutralityHistory?> getWholeHistory(int page,
    ...
    try {
      final uuid = GetIt.I
          ...
          .getString(StorageKeys.clientUuid);
      final Response<Map<String, dynamic>> response = await dio.get(
          '/reports/netNeutralityResult/history?page=$page&size=100&sort=measurementDate,desc&uuid=$uuid');
