...
void main() {
  ...
  group('Net neutrality API service', () {
    test('getSettings', () async {
      when(GetIt.I.get<Dio>().get('***REMOVED***')).thenAnswer(
    ...
    test('getSettings with error', () async {
     ...
     test('postResults', () async {
      when(GetIt.I
              .get<Dio>()
              .post('***REMOVED***', data: anyNamed('data')))
          .thenAnswer(
        (realInvocation) async => Response(
          requestOptions: RequestOptions(path: '***REMOVED***'),
          statusCode: 200,
    ...
    test('getHistory', () async {
      when(GetIt.I.get<Dio>().get(
              '/reports***REMOVED***/history?sort=measurementDate,desc&uuid=$_clientUuid&openTestUuid=$_openTestUuid'))
          .thenAnswer(
        (realInvocation) async => Response<Map<String, dynamic>>(
          requestOptions:
              RequestOptions(path: '/reports***REMOVED***/history'),
          statusCode: 200,
          data: _historyJson,
        ),
      );
