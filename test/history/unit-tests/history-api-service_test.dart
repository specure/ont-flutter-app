...
_setUpStubs() {
  ...
  when(dio.post(
    '/v2/mobile/history?page=1&size=100&sort=measurementDate,desc',
    data: {'uuid': 'uuid', 'devices': null, 'network_types': null},
  )).thenAnswer((_) async => Response(
        requestOptions: RequestOptions(path: '/mobile/history'),
        statusCode: 200,
        data: _historyJson,
      ));
  when(_errorHandler.process(_dioError)).thenReturn(null);
  when(dio.post(
    '/v2/mobile/history?page=2&size=100&sort=measurementDate,desc',
    data: {'uuid': 'uuid', 'devices': null, 'network_types': null},
  )).thenThrow(_dioError);
